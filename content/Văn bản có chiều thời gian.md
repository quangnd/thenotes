---
aliases:
  - Timeful text
created: 2026-05-26
modified: 2026-06-10
---

[[Phương tiện đại chúng hầu như thiếu chiều thời gian được tác giả thiết kế vượt quá một ngày]]. Đây có vẻ là một hướng thiết kế mạnh mẽ đáng khám phá, như ví dụ [[Ứng dụng thiền có hướng dẫn tự nhiên trải dài theo thời gian]] cho thấy.

Có thể gọi những văn bản được tác giả thiết kế để mở ra dần theo thời gian là *văn bản có chiều thời gian* (timeful texts).

- [[Phương tiện ghi nhớ giữ người đọc tiếp xúc với tài liệu theo thời gian]]
- [[Phương tiện ghi nhớ có thể được thiết kế để mở ra trải nghiệm theo thời gian]]
	- [[Văn bản ghi nhớ có thể được chia thành các phần nhỏ theo thời gian, đồng bộ với khả năng nhớ lại của người đọc]] (như [[Các bài học của Execute Program không mở khóa cho đến khi bạn đã ôn tập thành công các bài tiên quyết]])

---

> [!info]- Tài liệu tham khảo (References)
> [[Văn bản theo thời gian]]
