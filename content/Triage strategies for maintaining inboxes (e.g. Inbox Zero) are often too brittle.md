---
aliases:
  - Các chiến lược phân loại hộp thư đến (ví dụ Inbox Zero) thường quá mong manh
---

[[Inboxes only work if you trust how they’re drained|Hộp thư đến chỉ hữu ích khi bạn tin vào cách nó được xử lý]], và [Inbox Zero](http://www.43folders.com/izero) là một cách để đảm bảo điều đó. Nó giảm thời gian chờ của mỗi mục (về lý thuyết là xuống còn một ngày) bằng cách đẩy mạnh tốc độ xử lý.

Trong *Getting Things Done*, David Allen gợi ý rằng bạn có thể tăng tốc độ xử lý hàng đợi bằng cách hoãn, ủy quyền, hoặc bỏ qua nhiệm vụ một cách có chiến lược. "Inbox Zero" là phần bổ sung của Merlin Mann. Nó đảm bảo bạn đẩy tốc độ xử lý lên *đủ* nhanh, bằng cách dọn sạch hộp thư đến về số không mỗi ngày. Cách làm này hơi thô, nhưng giữ cho tốc độ xử lý luôn vượt tốc độ nhận.

### Thách thức
Bạn phải ra quyết định cho từng mục trong hộp thư đến. Đây là chi phí không nhỏ, và chỉ hợp lý khi hộp thư đến tương đối nhỏ.

Hoãn một nhiệm vụ một cách dứt khoát tạo ra chi phí cảm xúc không cần thiết. "Inbox zero" chỉ thật sự cần khi tốc độ nhận *luôn luôn* vượt tốc độ xử lý. Nếu tốc độ nhận thay đổi và đôi khi thấp hơn tốc độ xử lý, bạn vẫn xử lý kịp trong khung thời gian hợp lý.

Bỏ qua một nhiệm vụ một cách dứt khoát cũng khó, vì [[Software interfaces often harmfully frame destructive operations as final decisions, not contingent preferences|Giao diện phần mềm hay khung các thao tác hủy/xóa thành quyết định cuối cùng, thay vì lựa chọn tạm thời]].

Thực tế hơn, Inbox Zero hay dẫn đến tê liệt vì cứ hoãn mãi. Quá dễ để đẩy một nhiệm vụ sang ngày khác, rồi ngày khác nữa. Allen gợi ý nên rà soát định kỳ những nhiệm vụ bị hoãn nhiều lần, nhưng việc rà soát này lại đòi hỏi thêm hàng loạt quyết định. Mức độ tuân thủ thường thấp.

Khi xử lý hộp thư đến kiểu này, cũng có áp lực phải *làm* nhiều việc hơn, dù có thể đó không phải là điều bạn thật sự muốn.

### Cơ hội
Một cơ chế lý tưởng hơn sẽ giữ thời gian chờ ở mức chấp nhận được, nhưng không nhất thiết phải đóng chu kỳ trong một ngày. Cũng cần tính đến số lượng quyết định phải đưa ra. Cá nhân tôi thà ra ít quyết định hơn và chấp nhận thời gian chờ trung bình dài hơn.

Một cách hiện thực hóa khả thi: [[Spaced repetition can lower the stakes around destructive inbox-maintenance operations|Lặp lại ngắt quãng giúp giảm áp lực khi xóa bỏ trong hộp thư đến]].

---

#### Tài liệu tham khảo (References)
Allen, D. (2015). Getting Things Done: The Art of Stress-Free Productivity.

[43 Folders Series: Inbox Zero | 43 Folders](http://www.43folders.com/izero)

Matuschak, A. (2019, December). *Taking knowledge work seriously*. Trình bày tại Stripe Convergence, San Francisco.
